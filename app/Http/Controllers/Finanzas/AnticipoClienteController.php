...
class AnticipoClienteController extends Controller
{
    ...
    public function aplicar(Request $request, ClienteAnticipo $anticipo)
    {
        ...
        $data = $request->validate([
            ...
            'metodo_pago_id' => [$esMaterial ? 'nullable' : 'required', 'integer', Rule::exists('metodos_pago', 'id')->where('empresa_id', $user->empresa_id)],
            'cuenta_id'      => ['nullable', 'integer', Rule::exists('cuentas', 'id')->where('empresa_id', $user->empresa_id), $this->reglaCuentaObligatoria($request)],
        ]);
     ...
     * creado por el POS ("pendiente por entregar" o "despacho en almacén").
     * El usuario elige cuánto entrega de cada ítem; el stock saliente se
     * descuenta en el almacén del local de la venta.
     */
